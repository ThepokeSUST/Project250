...
//                     // Extract existing password
//                     String[] parts = line.split(",", 2); // Split only up to password
//                     String password = parts[1].split(",", 2)[0]; 
...
//         } catch (UnsatisfiedLinkError e) {
//             System.err.println("Failed to load OpenCV library. Ensure the native libraries are correctly configured in your system path.");
//             // You might want to display an error dialog here instead of just exiting
//             return; 
//         }
...
//     /**
//      * Cleans up camera resources and stops the camera thread.
//      */
//     private void stopLiveFeedAndReleaseCamera() {
//         showingLiveFeed = false;
//         if (cameraThread != null && cameraThread.isAlive()) {
//             try {
//                 // Interrupt the thread and wait a short time for it to stop
//                 cameraThread.interrupt(); 
//                 cameraThread.join(100); 
//             } catch (InterruptedException ignored) {
//                 Thread.currentThread().interrupt();
...
//         // =========================================================================
//         // ### MODIFIED BACKGROUND LOADING LOGIC WITH FALLBACK (Using background-png.png) ###
//         // =========================================================================
//         final String BACKGROUND_IMAGE_NAME = "background-png.png"; // <-- Confirmed File Name
//         String[] possiblePaths = {
//             BACKGROUND_IMAGE_NAME, 
//             "resources/" + BACKGROUND_IMAGE_NAME, 
//             "image_37c816.png", // Previous name fallback
//             "resources/image_37c816.png",
//             "bg.jpg", 
//             "resources/bg.jpg"
//         };
//         Image bgImage = null;
...
//                 // Ignore exceptions on file path attempts
//             }
//         }
...
//             );
//             root.setBackground(new Background(backgroundImage));
//             System.out.println("Background image '" + BACKGROUND_IMAGE_NAME + "' loaded successfully from: " + loadedPath);
//         } else {
//             // Last resort: try loading from classpath if file system failed (for IDEs)
//             try {
//                  bgImage = new Image(getClass().getResource("/" + BACKGROUND_IMAGE_NAME).toExternalForm());
//                  BackgroundImage backgroundImage = new BackgroundImage(
//                         bgImage, BackgroundRepeat.NO_REPEAT, BackgroundRepeat.NO_REPEAT,
//                         BackgroundPosition.CENTER, new BackgroundSize(
//                         BackgroundSize.AUTO, BackgroundSize.AUTO, true, true, true, true)
//                  );
//                  root.setBackground(new Background(backgroundImage));
//                  System.out.println("Background image '" + BACKGROUND_IMAGE_NAME + "' loaded successfully from CLASSPATH (IDE).");
//             } catch (Exception e) {
//                  System.err.println("❌ CRITICAL ERROR: Could not load background image using any method. Using solid color.");
//             }
//         }
//         // =========================================================================
...
//     /**
//      * Starts a dedicated thread to continuously read frames from the camera and update the ImageView.
//      */
//     private void startLiveFeed() {
//         showingLiveFeed = true;
//         Task<Void> cameraTask = new Task<>() {
...
//                     }
//                     try { 
//                         // Control frame rate (approx 30fps)
//                         Thread.sleep(33); 
//                     } catch (InterruptedException ignored) {
//                         // Cleanly exit the loop if interrupted
//                         break; 
//                     } 
//                 }
//                 frame.release();
...
//         };
//         cameraThread = new Thread(cameraTask); 
//         cameraThread.setDaemon(true); // Allow the application to exit even if this thread is running
//         cameraThread.start();
//     }
...
//     /**
//      * Captures a single frame, stops the live feed, saves the frame, and updates button states.
//      */
//     private void takePicture(Button takePicBtn, Button retakeBtn, Button analyzeBtn) {
//         if (camera == null || !camera.isOpened()) return;
...
//     /**
//      * Resumes the live feed and resets button states.
//      */
//     private void retakePicture(Button takePicBtn, Button retakeBtn, Button analyzeBtn) {
//         moodLabel.setText("📷 Ready to capture");
//         if (camera == null || !camera.isOpened()) {
//              // Re-initialize the scene if camera was closed
//              showMainApplicationScene(UserAuthenticator.DEFAULT_MOOD, UserAuthenticator.DEFAULT_TIME); 
//              return;
//         }
...
//     /**
//      * Initiates the mood analysis process on a captured frame using a background task.
//      */
//     private void analyzeMoodFromCapture() {
//         if (lastCapturedFrame == null) {
//             moodLabel.setText("⚠️ No image captured yet!");
...
//             @Override
//             protected String call() throws Exception {
//                 // Pass a clone to the analysis function to be safe
//                 Mat frameCopy = lastCapturedFrame.clone();
//                 return detectAndAnalyzeMood(frameCopy); // This mutates frameCopy and returns mood
//             }
...
//                 // 1. Update mood history list
//                 MoodEntry newEntry = new MoodEntry(currentMood, LocalDateTime.now());
//                 moodHistory.add(0, newEntry); 
//                 historyListView.refresh(); 

//                 // 2. Re-run analysis on a clone just for the UI update (to draw rectangles)
//                 Mat frameForDisplay = lastCapturedFrame.clone();
//                 detectAndAnalyzeMood(frameForDisplay); 
//                 Image processed = matToImage(frameForDisplay);
//                 imageView.setImage(processed);
...
//     /**
//      * Checks if the required cascade and model files exist in the current directory.
//      */
//     private static boolean checkRequiredFilesExist() {
//         boolean allFound = true;
//         if (!Files.exists(Paths.get(FACE_PROTO))) { System.err.println("Missing: " + FACE_PROTO); allFound = false; }
...
//     /**
//      * Performs face detection and initiates heuristic mood analysis. Draws bounding boxes on the image.
//      */
//     private static String detectAndAnalyzeMood(Mat image) {
//         Mat grayImage = new Mat();
//         Imgproc.cvtColor(image, grayImage, Imgproc.COLOR_BGR2GRAY);
...
//         if (faceNet != null) {
//             // Use DNN for robust face detection
//             Mat blob = Dnn.blobFromImage(image, 1.0, new Size(300, 300),
//                     new Scalar(104.0, 177.0, 123.0), false, false);
//             faceNet.setInput(blob);
//             Mat detections = faceNet.forward();
...
//             detections.release();
//         } else {
//             // Fallback to Haar Cascade if DNN loading failed (though we check this before)
//             CascadeClassifier faceDetector = new CascadeClassifier(FACE_PROTO); 
//             MatOfRect faceDetections = new MatOfRect();
//             faceDetector.detectMultiScale(grayImage, faceDetections);
//             facesArray = faceDetections.toArray();
...
//     /**
//      * Heuristic analysis based on smile and eye detection within the detected face region.
//      */
//     private static String analyzeMoodHeuristic(Mat faceRegionGray, int faceWidth) {
//         MatOfRect smileDetections = new MatOfRect();
//         // Look for smiles only in the lower half of the face
//         Rect lowerFace = new Rect(0, faceRegionGray.height() / 2, faceRegionGray.width(), faceRegionGray.height() / 2);
//         Mat smileRegion = faceRegionGray.submat(lowerFace);
...
//         MatOfRect eyeDetections = new MatOfRect();
//         // Look for eyes only in the upper half of the face
//         Rect upperFace = new Rect(0, 0, faceRegionGray.width(), faceRegionGray.height() / 2);
//         Mat eyeRegion = faceRegionGray.submat(upperFace);
...
//             mood = "Neutral"; 
//         else
//             mood = "Pensive"; // Default for non-happy/non-neutral (e.g., surprised, sad, confused)
...
//     /**
//      * Returns a color for the bounding box based on the detected mood.
//      */
//     private static Scalar getMoodColor(String mood) {
...
//     private static BufferedImage matToBufferedImage(Mat original) {
//         MatOfByte mob = new MatOfByte();
//         // Encode the Mat to a byte array (as JPEG)
//         Imgcodecs.imencode(".jpg", original, mob);
//         try {
//             // Read the byte array into a BufferedImage
//             return ImageIO.read(new ByteArrayInputStream(mob.toArray()));
...
//     /**
//      * Converts an OpenCV Mat object to a JavaFX Image object.
//      */
//     private static Image matToImage(Mat mat) {
//         BufferedImage bufferedImage = matToBufferedImage(mat);
